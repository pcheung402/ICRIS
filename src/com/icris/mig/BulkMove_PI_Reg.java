...
//		System.out.println("Hello world!");
		...
		File folder = new File("data/bulkMoveBatches_PI_Reg/batchSets/" + batchSetId);
...
//		intialSASize = revampedCPEUtil.getStorageAreaSize();
		...
		for (File fd : listOfFiles) {
			es.execute(new BulkMoverManager_PI_Reg(batchSetId, fd.getName(), log, bulkMoveOutputDataFile,  revampedCPEUtil));
		}		
		es.shutdown();
		try {
...
//			});
	...
	private static void initialize(String[] args) {
		...
		try {
			log = new ICRISLogger(batchSetId,"bulkMoveBatches_PI_Reg");
			...
			String bulkMoveOutputFilePath = "." + File.separator + "data" + File.separator + "bulkMoveOutput_PI_Reg" + File.separator + batchSetId +".dat";
